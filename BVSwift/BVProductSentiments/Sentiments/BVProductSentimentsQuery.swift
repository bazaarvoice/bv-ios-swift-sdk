  ...
  public func handler(completion: @escaping ((Response) -> Void)) -> Self {
    ...
    responseHandler = { [weak self] in
      ...
      case let .success(_, jsonData):
        ...
        guard let response: BVProductSentimentsQueryResponseInternal<BVType> =
          try? JSONDecoder()
            .decode(
              BVProductSentimentsQueryResponseInternal<BVType>.self,
              from: jsonData) else {
                completion(
                  .failure(
          ...
          guard let productSentiments = response.productSentiments else {
              completion(
              .failure(
                  [BVCommonError.unknown(
                      "An Unknown parse error occurred")])
        ...
        completion(.success(productSentiments))
          self?.productSentimentsPostflight(response.productSentiments)
